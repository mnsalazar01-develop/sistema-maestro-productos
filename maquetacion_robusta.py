...
st.title("🎨 Maquetador de Ofertas — Slots Fijos & Sin tocar Precios")
...
# ==============================================================================
# 9. GRID VISUAL DE SLOTS (FIJOS, NO SE MUEVEN)
# ==============================================================================
st.markdown("### 🎨 Cuadrante de Diseño — Arrastra no, clica sí")
st.caption("Cada slot es una caja fija. Usa los botones dentro de cada slot para quitar o asignar ofertas.")

ofertas = st.session_state.get("ofertas", [])
ofertas_libres = [o for o in ofertas if safe_int(o.get("numero_pagina")) is None]

# Mapear qué oferta está en cada slot de la página actual
slot_a_oferta = {}
for o in ofertas:
    if safe_int(o.get("numero_pagina")) == pag_act and safe_int(o.get("posicion_slot")) is not None:
        slot_a_oferta[safe_int(o["posicion_slot"])] = o

# Renderizar grid de slots
slot_num = 1
while slot_num <= slots_deseados:
    cols = st.columns(num_cols_reales)
    for c in cols:
        if slot_num > slots_deseados:
            break
        with c:
            with st.container(border=True):
                st.markdown(f"**📍 Slot {slot_num}**")
                oferta_en_slot = slot_a_oferta.get(slot_num)

                if oferta_en_slot:
                    # Mostrar oferta asignada
                    img_url = oferta_en_slot.get("img") or "https://picsum.photos/100"
                    nombre = oferta_en_slot.get("nombre", "Sin nombre")
                    precio = float(oferta_en_slot.get("precio_oferta", 0)) if oferta_en_slot.get("precio_oferta") is not None else 0

                    st.markdown(f"""
                        <div style="display:flex; align-items:center; gap:10px; margin:8px 0;">
                            <img src="{img_url}" style="width:50px; height:50px; object-fit:cover; border-radius:6px;" onerror="this.src='https://picsum.photos/100'">
                            <div style="font-size:13px; line-height:1.3;">
                                <div style="font-weight:600; color:#1e293b;">{nombre}</div>
                                <div style="color:#16a34a; font-weight:700;">${precio:,.0f}</div>
                            </div>
                        </div>
                    """, unsafe_allow_html=True)

                    if st.button("❌ Quitar", key=f"quitar_{slot_num}_{pag_act}", use_container_width=True):
                        for o in st.session_state.ofertas:
                            if o["id_oferta"] == oferta_en_slot["id_oferta"]:
                                o["numero_pagina"] = None
                                o["posicion_slot"] = None
                                break
                        st.success(f"Oferta devuelta al banco")
                        st.rerun()
                else:
                    # Slot vacío → selector para asignar
                    st.markdown("<div style='color:#94a3b8; font-size:12px; text-align:center; padding:10px 0;'>Vacío</div>", unsafe_allow_html=True)

                    if ofertas_libres:
                        opciones = {f"{o['id_oferta']} - {o['nombre']}": o for o in ofertas_libres}
                        sel = st.selectbox(
                            "Asignar:",
                            options=["— Elegir —"] + list(opciones.keys()),
                            key=f"asig_sel_{slot_num}_{pag_act}",
                            label_visibility="collapsed"
                        )
                        if sel != "— Elegir —":
                            oferta_sel = opciones[sel]
                            if st.button("➕ Asignar", key=f"asig_btn_{slot_num}_{pag_act}", use_container_width=True):
                                for o in st.session_state.ofertas:
                                    if o["id_oferta"] == oferta_sel["id_oferta"]:
                                        o["numero_pagina"] = pag_act
                                        o["posicion_slot"] = slot_num
                                        break
                                st.success(f"✅ Asignado a Slot {slot_num}")
                                st.rerun()
                    else:
                        st.caption("Sin ofertas libres")

        slot_num += 1

# ==============================================================================
# 10. BANCO DE OFERTAS LIBRES (VISUAL)
# ==============================================================================
if ofertas_libres:
    st.markdown("### 📦 Banco de Ofertas Disponibles")
    banco_cols = st.columns(min(len(ofertas_libres), 6))
    for idx, o in enumerate(ofertas_libres):
        with banco_cols[idx % 6]:
            with st.container(border=True):
                img_url = o.get("img") or "https://picsum.photos/100"
                nombre = o.get("nombre", "Sin nombre")
                precio = float(o.get("precio_oferta", 0)) if o.get("precio_oferta") is not None else 0
                st.markdown(f"""
                    <div style="text-align:center;">
                        <img src="{img_url}" style="width:60px; height:60px; object-fit:cover; border-radius:8px; margin-bottom:6px;" onerror="this.src='https://picsum.photos/100'">
                        <div style="font-size:11px; font-weight:600; color:#1e293b; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{nombre}</div>
                        <div style="font-size:11px; color:#16a34a; font-weight:700;">${precio:,.0f}</div>
                    </div>
                """, unsafe_allow_html=True)
                # Selector rápido de slot
                slot_opts = [s for s in range(1, slots_deseados + 1) if s not in slot_a_oferta]
                if slot_opts:
                    sel_slot = st.selectbox("Slot:", slot_opts, key=f"banco_slot_{o['id_oferta']}_{pag_act}", label_visibility="collapsed")
                    if st.button("➕", key=f"banco_asig_{o['id_oferta']}_{pag_act}", use_container_width=True):
                        for ofer in st.session_state.ofertas:
                            if ofer["id_oferta"] == o["id_oferta"]:
                                ofer["numero_pagina"] = pag_act
                                ofer["posicion_slot"] = sel_slot
                                break
                        st.rerun()
                else:
                    st.caption("Sin slots libres")
else:
    st.info("🎉 Todas las ofertas están asignadas.")
...
# ==============================================================================
# 13. GUARDAR EN SUPABASE — SIN TOCAR precio_oferta
# ==============================================================================
    ...
    for o in st.session_state.get("ofertas", []):
        ...
        if num_pag is not None and pos_slot is not None:
            ...
            lote_para_guardar.append({
                ...
                "numero_pagina": num_pag,
                "posicion_slot": pos_slot,
                # ❌ SIN "precio_oferta": o.get("precio_oferta")  ← ELIMINADO
