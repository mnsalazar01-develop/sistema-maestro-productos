...
# ==============================================================================
# 1. COMPONENTE CUSTOM DND
# ==============================================================================
parent_dir = os.path.dirname(os.path.abspath(__file__))
build_dir = os.path.join(parent_dir, "frontend")
# Fallback: si no encuentra la carpeta frontend al lado, busca en el directorio actual
if not os.path.isdir(build_dir):
    build_dir = os.path.join(os.getcwd(), "frontend")

dnd_component = components.declare_component("dnd_maquetador", path=build_dir)
...
# ==============================================================================
# 4. CAMPANAS (PAGINACIÓN BYPASS 1000)
# ==============================================================================
try:
...
columnas_css, filas_css, num_cols_reales = calcular_layout_grid(slots_deseados)

# ==============================================================================
# 10. COMPONENTE DND OPERATIVO
# ==============================================================================
st.markdown("### 🎨 Arrastra ofertas desde el banco hacia los slots")

# Preparar datos limpios para el componente (solo lo que necesita)
ofertas_para_componente = []
for o in st.session_state.get("ofertas", []):
    ofertas_para_componente.append({
        "id_oferta": o["id_oferta"],
        "nombre": o.get("nombre", ""),
        "img": o.get("img", "https://picsum.photos/100"),
        "precio_oferta": float(o.get("precio_oferta", 0)) if o.get("precio_oferta") is not None else 0,
        "numero_pagina": o.get("numero_pagina"),
        "posicion_slot": o.get("posicion_slot"),
    })

# Llamar al componente custom
resultado_dnd = dnd_component(
    ofertas=ofertas_para_componente,
    pagina=pag_act,
    num_slots=slots_deseados,
    cols=columnas_css,
    rows=filas_css,
    key=f"dnd_{pag_act}_{slots_deseados}"
)

# ==============================================================================
# 11. PROCESAR RETORNO DEL COMPONENTE
# ==============================================================================
if resultado_dnd:
    try:
        evento_ts = resultado_dnd.get("timestamp", 0)
        if evento_ts > st.session_state.ultimo_ts_procesado:
            id_mod = resultado_dnd.get("id_oferta")
            nueva_p = resultado_dnd.get("numero_pagina")
            nuevo_s = resultado_dnd.get("posicion_slot")

            cambio = False
            for ofer in st.session_state.ofertas:
                if ofer["id_oferta"] == id_mod:
                    # Si se devolvió al banco (slot 0 o pagina 0)
                    if nuevo_s == 0 or nueva_p == 0:
        ...
        st.error(f"Error procesando drag & drop: {e}")

# ==============================================================================
# 12. TABLA DE ASIGNADAS
# ==============================================================================
...
# ==============================================================================
# 13. DESASIGNAR MANUAL (RESPALDO)
# ==============================================================================
ofertas_en_pagina = [o for o in st.session_state.get("ofertas", []) if safe_int(o.get("numero_pagina")) == pag_act]

if ofertas_en_pagina:
    st.markdown("#### 🗑️ Desasignar manualmente")
    cols_dev = st.columns(min(len(ofertas_en_pagina), 4))
    for idx, o in enumerate(ofertas_en_pagina):
        with cols_dev[idx % 4]:
            nombre_corto = o.get("nombre", "")[:22]
            if st.button(f"❌ {nombre_corto}... (S{o.get('posicion_slot', '?')})", key=f"dev_{o['id_oferta']}_{pag_act}"):
                for ofer in st.session_state.ofertas:
                    if ofer["id_oferta"] == o["id_oferta"]:
                        ofer["numero_pagina"] = None
                        ofer["posicion_slot"] = None
                        break
                st.success("Oferta devuelta al banco.")
                st.rerun()

# ==============================================================================
# 14. PREPARAR LOTE PARA GUARDAR
# ==============================================================================
filas_desasignadas = []
for o in st.session_state.get("ofertas", []):
    num_pag = safe_int(o.get("numero_pagina"))
